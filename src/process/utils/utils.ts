...
export const getTempPath = () => {
  ...
  return path.join(rootPath, 'wepulse-hermes');
};
 ...
 * Release builds use ~/.wepulse-hermes; dev builds use ~/.wepulse-hermes-dev.
 * 获取数据目录路径，macOS 上使用符号链接。
 * Release 使用 ~/.wepulse-hermes，Dev 模式使用 ~/.wepulse-hermes-dev。
 */
export const getDataPath = (): string => {
  const rootPath = getElectronPathOrFallback('userData');
  const dataPath = path.join(rootPath, 'wepulse-hermes');
  return ensureCliSafeSymlink(dataPath, getEnvAwareName('.wepulse-hermes'));
};

/**
 * Get config path, using CLI-safe symlink on macOS.
 * Release builds use ~/.wepulse-hermes-config; dev builds use ~/.wepulse-hermes-config-dev.
 * 获取配置目录路径，macOS 上使用符号链接。
 * Release 使用 ~/.wepulse-hermes-config，Dev 模式使用 ~/.wepulse-hermes-config-dev。
 */
export const getConfigPath = (): string => {
  const rootPath = getElectronPathOrFallback('userData');
  const configPath = path.join(rootPath, 'config');
  return ensureCliSafeSymlink(configPath, getEnvAwareName('.wepulse-hermes-config'));
};
