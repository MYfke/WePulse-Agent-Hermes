...
export function createMainMenuCard(): LarkCard {
  return {
    ...
    header: {
      title: { tag: 'plain_text', content: `${APP_DISPLAY_NAME} Assistant` },
      template: 'blue',
    },
    elements: [
      {
    ...
    elements: [
      {
        ...
        content: [
          '**What is pairing?**',
          `Pairing links your Lark/Feishu account with the local ${APP_DISPLAY_NAME} application.`,
...
export function createHelpCard(): LarkCard {
  return {
    config: { wide_screen_mode: true },
    header: {
      title: { tag: 'plain_text', content: `❓ ${APP_DISPLAY_NAME} Assistant Help` },
      template: 'turquoise',
    },
    elements: [
      {
        tag: 'markdown',
        content: [
          `A remote assistant to interact with ${APP_DISPLAY_NAME} via Lark.`,
          '',
          '**Common Actions:**',
